-- 023_trainer_identity.sql — Trainer: player identity, party, credentials + RLS
-- Run manually in the Supabase SQL editor (this project's schema lives in the
-- dashboard; there is no CLI migration history).
--
-- NOT APPLIED — written for review. Database slice only: tables, constraints,
-- policies. No UI, no signal table, no Discord, no credential issuer.
--
-- ── Relationship to 018_trainer_identity.sql ─────────────────────────────────
-- 018 SUPERSEDED. It covers the same ground but models the identity axis as two
-- free-text columns (play_type, play_type_secondary). This slice replaces that
-- with identity_mode + a closed 30-value playstyle array + favorite_legends.
-- 018 is marked NOT APPLIED in its own header and 022's, so the expected state
-- is that none of these tables exist yet and this file is the one to run.
--
-- It is nonetheless written to be safe if 018 WAS applied: every table is
-- `create table if not exists`, and the new columns are added with
-- `alter table ... add column if not exists` immediately after, so a
-- half-migrated database converges instead of silently no-opping. 018's
-- play_type columns are left in place if present — dropping them would break
-- 022_trainer_connection.sql, which still selects them. See "Known gaps" #7.
--
-- Additive with respect to pre-existing schema. No existing table is altered;
-- `decks` is referenced by foreign key only.
--
-- Ordering follows default-deny: every table below has RLS enabled and its
-- policies written BEFORE any row is inserted. The handle_reservation seed at
-- the bottom is the only INSERT in this file, and it runs last.
--
...
-- Which axis this trainer chose to represent them. A flag, not a constraint:
-- both playstyle and favorite_legends may hold data simultaneously and neither
-- is cleared when the mode flips. Exactly one is RENDERED on the card and the
-- public profile, and that is a read-time decision — the database keeps whatever
-- the trainer typed so switching modes back does not lose their other answer.
do $$ begin
  create type trainer_identity_mode as enum ('playstyle', 'legends');
exception
  when duplicate_object then null;
end $$;
...
-- product. Both self-reported and verified credentials take the SAME write path
-- with different `method` values — one path is auditable, two are not.
do $$ begin
...
-- Needed because a CHECK constraint cannot contain a subquery, and there is no
-- array operator for "longest element". A function call in a CHECK is legal, so
-- the unnest lives here instead. IMMUTABLE (char_length is), which is what makes
-- it usable in a constraint at all.
--
-- Defined before `trainer` so a pg_dump restore creates it first.
create or replace function text_array_elements_within(arr text[], max_len int)
returns boolean
language sql
immutable
as $$
  -- unnest(null) yields zero rows, so bool_and is NULL for both a NULL array
  -- and an empty one; coalesce turns "nothing to check" into a pass.
...
create table if not exists trainer (
  ...
  -- Free text ("north side", "Twin Cities"). Coordinates are forbidden, so the
  ...
  -- Array, not enum-per-row: philosophies are a multi-select with no order, no
  -- scale, and deliberately NO cap on count — a trainer can honestly claim all
  -- four. `<@` = "is contained by", so an empty array passes and anything
  ...
  identity_mode trainer_identity_mode not null default 'playstyle',

  -- Closed vocabulary of 30, using the same `<@` idiom as philosophy above.
  -- Fixed on purpose: an unconstrained column drifts into 40 spellings of
  -- "+1/+1 counters" within a month, and the card layout has to render every
  -- one of them. Adding a value is a follow-up migration, which is the point —
  -- it makes the vocabulary a reviewed decision instead of a client typo.
  playstyle text[] default '{}'
    constraint trainer_playstyle_max
      check (playstyle is null or array_length(playstyle, 1) <= 3)
    constraint trainer_playstyle_allowed
  ...
  -- Free text per entry, unlike playstyle: the set of legendary creatures is
  -- ~2000 and grows every set, so a closed list here would be stale on release
  -- day. Cap is per element, not on the whole array.
  favorite_legends text[] default '{}'
    constraint trainer_favorite_legends_max
      check (favorite_legends is null or array_length(favorite_legends, 1) <= 3)
    constraint trainer_favorite_legends_elem_len
      check (text_array_elements_within(favorite_legends, 60)),

  visibility trainer_visibility not null default 'private',
  created_at timestamptz not null default now()
);

-- Convergence for a database where 018 already ran: add what this slice
-- introduces. No-ops on a fresh install where the create above did the work.
alter table trainer
  add column if not exists identity_mode trainer_identity_mode not null default 'playstyle';
alter table trainer
  add column if not exists playstyle text[] default '{}';
alter table trainer
  add column if not exists favorite_legends text[] default '{}';

-- Constraints have no `if not exists`, so they are added through a catalog
-- guard. Only reachable in the 018-already-applied case; on a fresh install the
-- create table above already named them and these skip.
do $$ begin
  if not exists (select 1 from pg_constraint where conname = 'trainer_playstyle_max') then
    alter table trainer add constraint trainer_playstyle_max
      check (playstyle is null or array_length(playstyle, 1) <= 3);
  end if;
  if not exists (select 1 from pg_constraint where conname = 'trainer_playstyle_allowed') then
    alter table trainer add constraint trainer_playstyle_allowed
      check (playstyle <@ array[
        'tokens', 'plus_one_counters', 'artifacts', 'combo', 'aggro',
        'lifegain', 'spellslinger', 'reanimator', 'aristocrats', 'lands_matter',
        'control', 'burn', 'equipment', 'ramp', 'enchantress',
  ...
  if not exists (select 1 from pg_constraint where conname = 'trainer_favorite_legends_max') then
    alter table trainer add constraint trainer_favorite_legends_max
      check (favorite_legends is null or array_length(favorite_legends, 1) <= 3);
  end if;
  if not exists (select 1 from pg_constraint where conname = 'trainer_favorite_legends_elem_len') then
    alter table trainer add constraint trainer_favorite_legends_elem_len
      check (text_array_elements_within(favorite_legends, 60));
  end if;
end $$;
...
-- What you'd actually sleeve up tonight. Distinct from the dex, and mutable —
-- deleting a slot does not touch the deck.
--
-- decks(id) confirmed against the live schema rather than guessed: `decks` is
-- dashboard-created (no migration defines it), and 002_legends.sql /
-- 008_one_deck_per_legend.sql / 013_multi_user.sql all reference decks.id as a
-- uuid with legend_id, build_name, status, and user_id alongside it.
create table if not exists party_slot (
  id uuid primary key default gen_random_uuid(),
  trainer_id uuid not null references trainer(id) on delete cascade,
  deck_id uuid not null references decks(id) on delete cascade,

  -- The hard cap of 5 lives HERE, not in client validation. With the
...
-- bracket and its own date. There is deliberately no function, view, or column
-- in this file that reduces them to a score, rank, or trust number.
create table if not exists credential (
  ...
  -- Surfaced in UI, never hidden. "Rank 7, verified 4 Mar" beats a bare number,
  -- and a stale ranking must read as stale rather than silently refresh.
  issued_at timestamptz not null,
  expires_at timestamptz,
  revoked_at timestamptz
);

-- 018 defined trust_level(uuid), which summed credentials into a single integer
-- (venue=4, api=2, self=1). That is exactly the aggregate this slice forbids, so
-- it is dropped rather than left orbiting the schema waiting to be called.
-- Nothing references it — 022 does not, and no client code does.
--
-- JUDGMENT CALL, flagged for review: delete this line if you want 018's function
-- kept. It is the only statement in this file that removes anything.
drop function if exists trust_level(uuid);
...
-- ── trainer_public ───────────────────────────────────────────────────────────
-- The ONLY path by which unauthenticated traffic reads trainer data. Not a
-- filtered `select *` on the private table: the column list here is the
...
--
-- Note what is absent: `visibility` itself, and `id`. See "Known gaps" #2.
--
...
--
-- Dropped rather than `create or replace`d: the column list differs from 018's
-- (identity_mode, playstyle, favorite_legends in; play_type,
-- play_type_secondary out), and replace cannot change a view's columns.
...
--    private ones. 022 proposes get_trainer_card() as the resolution.
--
-- 4. No handle-change rate limit. Needs a handle_changed_at column and a
--    trigger; the policy (1 per 30 days?) is unresolved.
--
-- 5. `philosophy` has no minimum-1 constraint — `default '{}'` means empty is
--    allowed. If the product requires at least one, that is a follow-up.
--
...
--
-- 7. 022_trainer_connection.sql (also unapplied) still returns play_type and
--    play_type_secondary from get_trainer_card() and my_roster(). Those columns
--    do not exist on a fresh install of this file, so 022 will fail to create
--    until its two function signatures are updated to identity_mode / playstyle
--    / favorite_legends. Not fixed here: 022 is outside this task's file scope.
--
-- 8. identity_mode is advisory. Nothing stops a trainer with
--    identity_mode='legends' from having a populated playstyle array, by design
--    (mode flips must not destroy data) — so every renderer has to honour the
--    flag itself. A view per mode would enforce it; that is a UI-layer call.
