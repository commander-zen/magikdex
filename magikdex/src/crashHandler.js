...
export function installGlobalCrashHandler() {
  ...
  window.addEventListener("error", (ev) => {
    const e = ev.error;
    show("MAGIKDEX FAILED TO START", String((e && (e.stack || e.message)) || ev.message || ev));
  });

  window.addEventListener("unhandledrejection", (ev) => {
    const r = ev.reason;
    show("MAGIKDEX FAILED TO START", String((r && (r.stack || r.message)) || r));
  });
